...
// 二分法向前查找目标值位置
func (this optSliceInt64) FindAcs(dest int64) (index int) {
...
// 二分法向后查找目标值位置
func (this optSliceInt64) FindDesc(dest int64) (index int) {
	lo, hi := 0, len(this.slc)-1
	for lo <= hi {
		m := (lo + hi) >> 1
		if this.slc[m] < dest {
...
// 查找目标值第一次出现的位置
func (this optSliceInt64) Index(dest int64) (index int) {
	for i := 0; i < len(this.slc); i++ {
		if this.slc[i] == dest {
			return i
		}
...
// 查找目标值最后一次出现的位置
func (this optSliceInt64) LastIndex(dest int64) (lastIndex int) {
